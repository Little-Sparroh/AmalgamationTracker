...
public static class BossTimerPatches
{
    ...
    {
        if (BossTimerHUD.Instance != null) BossTimerHUD.Instance.ResetTimer();
    }

    [HarmonyPatch(typeof(AmalgamationObjective), "OnAmalgamationSpawned_ClientRpc")]
    [HarmonyPostfix]
    private static void OnAmalgamationSpawned_ClientRpc_Postfix(AmalgamationObjective __instance,
        NetworkBehaviourReference brainRef)
    {
        if (BossTimerHUD.Instance != null) BossTimerHUD.Instance.StartTimer();
    }
    ...
    {
        if (BossTimerHUD.Instance != null) BossTimerHUD.Instance.StopTimer();
    }
}